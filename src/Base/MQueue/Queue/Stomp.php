...
{
	private \Stomp\StatefulStomp $client;
	 ...
	 *
	 * @param \Stomp\StatefulStomp $client StatefulStomp object
	 * @param string $queue Message queue name
	 * @throws \Aimeos\Base\MQueue\Exception
	 */
	public function __construct( \Stomp\StatefulStomp $client, string $queue )
	{
		$this->subscriptionId = $client->subscribe( $queue );
	...
	{
		if( $this->client->send( $this->queue, new \Stomp\Transport\Message( $msg ) ) === false )
		{
